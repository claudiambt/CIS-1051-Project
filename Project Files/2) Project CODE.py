...
clustalolocation= "/opt/homebrew/bin/clustalo" #so that program knows where to find clustalo, location on my computer
...
insects = ["FruitFly", "Butterfly"] #the outgrup
...
Phylo.write(treecytb, "cytbtree.xml", "phyloxml") #save tree info file
treeef = treeconstructor(alignedsequenceef, insects)
Phylo.write(treeef, "eftree.xml", "phyloxml")

from Bio.Align import MultipleSeqAlignment

def concatenate(align1, align2): #to combine cytb + eef1a2
    combinedSequences = [] #the new combined sequences will be stored here
    
    for i in range(len(align1)): #loops over the number of species
        rec1 = align1[i] #gets i from cytb
        rec2 = align2[i] #gets i from ef

        newSeq = rec1.seq + rec2.seq #combines sequences
        rec = rec1[:]         # copy rec1 so we can replace its sequence without changing the original rec1 alignment
...
alignmentfile = "concatenatedAlignment.fasta"
...
def iqtree(iqtree, aligmnentfile):
    ...
    with open("constraint.nwk", "w") as f:
        f.write(constrainttext)

    cmd = [
        iqtreelocation,
        "-s", alignmentfile,
        "-o", "FruitFly,Butterfly", #set the outgroup
        "-g", "constraint.nwk", 
        "-m", "TESTNEW", #set the model that will be used
        "-bb", "1000",
        "-alrt", "1000",
    ...
    treefile = alignmentfile + ".treefile"
    tree = Phylo.read(treefile, "newick")#bring the tree "back" from IQtree to Python
    Phylo.draw_ascii(tree)
    return tree

finaltree = iqtree(iqtreelocation, alignmentfile)
Phylo.write(finaltree, "IQtree.xml", "phyloxml")

import matplotlib
import matplotlib.pyplot as plt

def drawtree (asciitree, title): #draw tree using matplotlib
...
drawtree(treecombined,"cytb + eef1a2")
